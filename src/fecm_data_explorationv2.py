...
"""
The AEGIS Inventory Builder achieves three objectives
...
def main(sector,filename_flag,naics,corrected_parquet_available_flag,flag_for_running_stewi,year):    
    ...
    flag_for_running_stewi: boolean
    year: str
    ...
    out_path3 = out_path1 + "/" + sector + "/" + "issue_files/"
    try:
        os.mkdir(out_path2)
        os.mkdir(out_path3)
    ...
    def naics_code_checker(df1,frs_id_list):
        ...
        fac_with_naics_code_issue.to_csv(out_path3+sector+'_'+'facilities_with_naics_code_issues.csv', index=False)
    ...
    def manipulate_databases(naics_code1, naics_code2, naics_code3, naics_code4):
        ...
        missing_in_flow_but_not_fac.to_csv(out_path3+'missing_in_combined_flow_but_not_facility_level_inventory.csv', index=False)
        ...
        if filename_flag:
            ...
            ghgrp_raw_data = pd.read_excel('../Data/GHGRP_epa_'+year+'.xlsx')
            ...
            df_ghgrp_flight = df_ghgrp_flight.merge(
                ghgrp_raw_data, left_on=['GHGRP ID'], right_on=['Facility Id'])
            ...
            #Creating the inventory using the FRS ID list. Even if NAICS code is wrong it will get selected
            df1_sector = df1_air.loc[df1_air['FRS_ID'].isin( frs_id_list_all_db)]
    ...
    def facility_stack_parameters(df1_sector):
        ...
        try:
            ...
            flat_fac.to_csv(out_path2+sector+'_'+'nei_fac_level_info_for_stack_parameters.csv', index = False)
            ...
            flat_unit.to_csv(out_path2+sector+'_'+'nei_unit_level_info_for_stack_parameters.csv', index = False)
            return None
        except:
            print('File not found: reduced_stack_file.csv > 300 MB. Please ask authors for the file to create inventory with stack parameters')
    ...
    def overlap_explorer(df1_sector):
        ...
        overlap_df.to_csv(out_path3+sector+'_'+'overlap_explorer_with_facility_names.csv', index = False)
        ...
        if not overlap_df_corr.empty:
            overlap_df_corr.to_csv(out_path3+sector+'_'+'overlap_explorer_with_facility_names.csv', index=False)
        
            del overlap_df_corr['FacilityName']
            overlap_df_w_name = overlap_df_corr
            overlap_df_w_name.to_csv(
            out_path3+sector+'_'+'overlap_explorer_without_facility_names.csv', index=False)
    ...
    def plots_and_exploration1(sector_df2):
        '''
        This function plots and explores cap emissions in details. it explores the concentration 
        ...
        for em in cap_emissions:
            ...
            '''
        sector_inv_for_cap_em.to_csv(out_path2+sector+'_facility_level_cap_emissions.csv',index=False)
    ...
    def plots_and_exploration2(sector_df3):
        ...
        sector_inv_for_cap_em.to_csv(out_path2+sector+'_facility_level_cap_emissions_backcalculated.csv',index=False)
    ...
    def ghgrp_process_exploration_unit(naics1, naics2, naics3, naics4):
        ...
        ghgrp_process_df.to_csv(
            out_path2+sector+'_'+'inventory_with_process_data_ghgrp_unit_level.csv', index=False)
    ...
    def ghgrp_process_exploration_fuel(naics1, naics2, naics3, naics4):
        ...
        ghgrp_process_df = ghgrp_process_df.merge(ghgrp_df, left_on = ['Facility Id'], right_on = ['FacilityID'], how = 'left').drop_duplicates()
        ghgrp_process_df.to_csv(
            out_path2+sector+'_'+'inventory_with_process_data_ghgrp_fuel_level.csv', index=False)
    ...
    def ghgrp():
        """
        This function reads the GHGRP data files, compares with the created inventory facility level files and creates issue files listing various issues
        showing up in the emissions inventory. This includes missing facilities and FRS mismatches. 
        ...
        Output
        ------
        Issue files as pandas dataframes in the csv format
        """
        ...
        missing_facilities.to_csv(
            out_path3+sector+'_'+'ghgrp_issue_frs_mismatch.csv', index=False)
        
        

        
        missing_facilities = f_sector_production.merge(df1_f_sector, left_on=[
                                                     'Facility Id'], right_on=['FacilityID'],how = 'outer').drop_duplicates()       
        ...
        issues_with_ghgrp.to_csv(
           out_path3+sector+'_'+'ghgrp_issue_missing_facility.csv', index=False)
    ...
    def nei(naics_list):
        """
        This function reads the NEI data files, compares with the created inventory facility level files and creates issue files listing various issues
        showing up in the emissions inventory. This includes missing facilities and FRS mismatches. 
        
        Parameters
        ----------
        naics_list: list
            list of naics code belonging to the industrial sector under study
        ...
        #Another way of extracting is using the FRS ID list from STEWI. However, what if the FRS IDs are wrong?
        ...
        missing_facilities2.to_csv(
            out_path3+sector+'_'+'nei_issue_missing_facility_using_naics_codes.csv', index=False)
    ...
    def tri(naics_list):
        """
        This function reads the TRI data files, compares with the created inventory facility level files and creates issue files listing various issues
        showing up in the emissions inventory. This includes missing facilities and FRS mismatches. 
        ...
        #Missing facility
        issues_with_tri.to_csv(out_path3+sector+'_'+'tri_issue_missing_facility_all_using_naics_code.csv', index=False)
        ...
        issues_with_tri_frs_issues.to_csv(
            out_path3+sector+'_'+'tri_issue_frs_mismatch.csv', index=False)    
        ...
        #Missing facility
        issues_with_tri_ghgrp_only.to_csv(out_path3+sector+'_'+'tri_issue_missing_facility_ghgrp_only.csv', index=False)
        #Missing completely in STEWI
        tri_missing_in_stewi.to_csv(out_path3+sector+'_'+'tri_missing_in_stewi.csv')
        #Missing only in combined files
        tri_missing_in_combination_frs_fac_id_issue.to_csv(out_path3+sector+'_'+'tri_missing_in_combination_frs_fac_id_issue.csv')
        #Other issues
        tri_missing_other_issues.to_csv(out_path3+sector+'_'+'tri_missing_other_issues.csv')
